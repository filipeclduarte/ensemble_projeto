...
plt.savefig('resultados/rmse_treinanmento_santa_fe.png')
plt.show()
...
ax.set_ylabel('Qtd. neurônios na camada de entrada')
ax.set_xlabel('Qtd. neurônios na camada de escondida')
plt.savefig('resultados/rmse_santa_fe.png')
plt.show()

# analisar configurações
print('Melhores configurações 100 modelos')
print(df_res.sort_values('RMSE'))
df_res.sort_values('RMSE').to_csv('resultados/df_res_santa_fe.csv')
...
# salvar resultados
df_res_ind.sort_values('RMSE').to_csv('resultados/df_res_ind_santa_fe.csv')
...
ax.set_ylabel('Qtd. neurônios na camada de entrada')
ax.set_xlabel('Qtd. neurônios na camada de escondida')
plt.savefig('resultados/rmse_best_model_ind_santa_fe.png')
plt.show()
...
for i,_ in enumerate(res_pool_size):
    res_pool_size[i].append(dados[30][150][i+1]['rmses'])
    res_pool_size[i].append(dados[30][150][i+1]['rmses_media'])
    res_pool_size[i].append(dados[30][150][i+1]['rmses_median'])
# analisar tabela
...
df_res_pool_size.to_csv('resultados/df_res_pool_size_santa_fe.csv')
...
plt.ylabel('RMSE')
plt.savefig('resultados/RMSE_pool_size_santa_fe.png')
plt.show()

## Gráfico de previsao do melhor modelo, melhor ensemble, média e mediana
res_pred = {'Y_teste': teste.values, 'Best ELM': dados[150][110][1]['predictions_pso'].reshape(-1), 
            'Ensemble': dados[30][150][100]['predictions_pso'].reshape(-1), 'Média': dados[30][150][100]['predictions_mean'],
            'Mediana': dados[30][150][100]['predictions_median']}
...
plt.xlabel('t')
plt.savefig('resultados/previsoes_santa_fe.png')
plt.show()

# salvar previsoes
df_res_pred.to_csv('resultados/previsoes_santa_fe.csv')